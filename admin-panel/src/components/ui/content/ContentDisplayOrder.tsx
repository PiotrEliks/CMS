...
import { usePageComponents, type PageComponent } from '../../../store/pageComponents';
...
export default function ContentDisplayOrder({
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      await Promise.all([fetchSections(contentId), fetchComponents(contentId, true)]);
    } catch (error) {
  ...
  const saveDisplayOrder = async (orderedItems: DisplayItem[]) => {
    ...
    if (componentUpdates.length > 0) {
      requests.push(
        api.post(`/components/contents/${contentId}/components/reorder-display`, {
  ...
  const getComponentTitle = (component: PageComponent): string => {
    ...
    return component.data?.title || typeLabels[component.component_type] || 'Komponent';
  };

  const getComponentSubtitle = (component: PageComponent): string | undefined => {
    ...
    <div className="space-y-4">
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          <strong>Kolejność wyświetlania na stronie publicznej:</strong> Przeciągnij i upuść
          elementy, aby zmienić ich kolejność. Możesz mieszać proste sekcje z komponentami
          PageBuilder.
        </p>
      </div>
      ...
      <div className="flex items-center gap-6 text-sm text-gray-600 dark:text-gray-400">
        <div className="flex items-center gap-2">
          ...
          <span>{items.filter((i) => i.type === 'section').length} sekcji</span>
        </div>
        <div className="flex items-center gap-2">
          <Box className="w-4 h-4 text-green-500" />
          <span>{items.filter((i) => i.type === 'component').length} komponentów</span>
        </div>
        <div className="flex items-center gap-2">
          <EyeOff className="w-4 h-4 text-yellow-500" />
          <span>{items.filter((i) => !i.status).length} ukrytych</span>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {items.map((item, index) => (
              <DisplayOrderItem
                ...
                index={index}
                onEdit={() => {
                  if (item.type === 'section' && item.section && onEditSection) {
                    onEditSection(item.section);
                  } else if (item.type === 'component' && item.component && onEditComponent) {
...
function DisplayOrderItem({ item, index, onEdit }: DisplayOrderItemProps) {
  ...
  const bgColor =
    item.type === 'section' ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-green-50 dark:bg-green-900/20';
  const iconColor = item.type === 'section' ? 'text-blue-600' : 'text-green-600';
