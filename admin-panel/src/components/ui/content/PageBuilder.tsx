...
export default function PageBuilder({ contentId }: PageBuilderProps) {
  const { components, loading, fetchComponents, reorderComponents } = usePageComponents();
  ...
  const visibleComponents = showInactive ? components : components.filter((c) => c.status);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {visibleComponents.length} komponent{visibleComponents.length === 1 ? '' : 'y'}
            </p>
          </div>
        ...
        {visibleComponents.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">
              Brak komponentów. Dodaj swój pierwszy komponent, aby zacząć.
            </p>
            <Button variant="primary" onClick={() => setTypeSelectorOpen(true)}>
              Dodaj komponent
            </Button>
          </div>
        ) : (
...
function getDefaultDataForType(type: ComponentType): any {
  switch (type) {
    case 'hero':
      return {
        slides: [{ title: 'Witaj', subtitle: '', buttonText: 'Zaczynaj', buttonLink: '#' }],
        autoplay: true,
        interval: 5000,
      };
    case 'services':
