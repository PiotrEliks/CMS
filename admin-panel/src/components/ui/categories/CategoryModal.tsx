...
export default function CategoryModal({ open, onClose, category, onSuccess }: CategoryModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Nie udało się zapisać kategorii');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full">
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-6">
                Nieaktywne kategorie nie są widoczne na stronie publicznej
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
            ...
            <Button variant="primary" startIcon={<Save />} onClick={handleSave} disabled={loading}>
              {loading ? 'Zapisywanie...' : isEdit ? 'Zapisz zmiany' : 'Utwórz kategorię'}
            </Button>
          </div>
        </div>
      </div>
