...
export default function ContactFormEditor({ data, onChange }: ContactFormEditorProps) {
  ...
  const addOption = (fieldIndex: number) => {
    ...
    updateField(fieldIndex, { options: [...options, `Opcja ${options.length + 1}`] });
  };
  ...
  const loadTemplate = (template: 'basic' | 'detailed') => {
    ...
    switch (template) {
      case 'basic':
        fields = [
          { type: 'text', name: 'name', label: 'Imię i nazwisko', required: true },
          { type: 'email', name: 'email', label: 'Adres email', required: true },
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button size="sm" variant="primary" startIcon={<Plus />} onClick={addField}>
            Dodaj pole
          </Button>
        </div>
