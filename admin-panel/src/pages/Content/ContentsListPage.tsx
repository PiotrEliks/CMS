...
export default function ContentsListPage() {
  ...
  const fetchContents = async () => {
    ...
    try {
      ...
      if (appliedFilters.created_by) params.created_by = appliedFilters.created_by;
      if (appliedFilters.updated_by) params.updated_by = appliedFilters.updated_by;

      if (appliedFilters.created_from) {
        params.created_from = appliedFilters.created_from.toISOString().split('T')[0];
      }
      if (appliedFilters.created_to) {
        params.created_to = appliedFilters.created_to.toISOString().split('T')[0];
      }
      if (appliedFilters.updated_from) {
        params.updated_from = appliedFilters.updated_from.toISOString().split('T')[0];
      }
      if (appliedFilters.updated_to) {
        params.updated_to = appliedFilters.updated_to.toISOString().split('T')[0];
      }
        ...
        >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) => handleFilterChange('created_from', date)}
                ...
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) => handleFilterChange('updated_from', date)}
                    dateFormat="dd.MM.yyyy"
                    placeholderText="Wybierz datę"
                    className="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white"
                    isClearable
          ...
          {hasActiveFilters && !showFilters && (
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              {appliedFilters.status && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Status: {appliedFilters.status === 'P' ? 'Opublikowane' : 'Szkic'}
                </span>
              )}
              {appliedFilters.created_by && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Utworzył:{' '}
                  {users.find((u) => u.user_id === appliedFilters.created_by)?.display_name ||
                    appliedFilters.created_by}
                </span>
              )}
              {appliedFilters.updated_by && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Edytował treść:{' '}
                  {users.find((u) => u.user_id === appliedFilters.updated_by)?.display_name ||
                    appliedFilters.updated_by}
                </span>
              )}
              {appliedFilters.created_from && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Utworzono od: {appliedFilters.created_from.toLocaleDateString('pl-PL')}
                </span>
              )}
              {appliedFilters.created_to && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Utworzono do: {appliedFilters.created_to.toLocaleDateString('pl-PL')}
                </span>
              )}
              {appliedFilters.updated_from && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Edytowano treść od: {appliedFilters.updated_from.toLocaleDateString('pl-PL')}
                </span>
              )}
              {appliedFilters.updated_to && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
                  Edytowano treść do: {appliedFilters.updated_to.toLocaleDateString('pl-PL')}
                </span>
              )}
            </div>
          )}
                    ...
                    {contents.map((content) => (
                      ...
                      >
                        <td className="py-3 px-4">
                          ...
                          {(content.has_sections || content.has_components) && (
                        ...
                        <td className="py-3 px-4">
                          ...
                          <div className="text-xs text-gray-400 dark:text-gray-500">
                            {getTimeSince(content.created_at)}
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          {content.updater ? (
                            <div className="text-sm">
                              <div className="font-medium text-gray-900 dark:text-white">
                                {content.updater.display_name}
                              </div>
                              <div className="text-gray-500 dark:text-gray-400 text-xs">
                                {content.updater.email}
                              </div>
                            </div>
                          ) : (
                        ...
                        <td className="py-3 px-4">
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                            {formatDate(content.last_edit_date)}
                          </div>
                          <div className="text-xs text-gray-400 dark:text-gray-500">
                            {getTimeSince(content.last_edit_date)}
                          </div>
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <Access allOf={['content.read_any']}>
                              <Link to={`/contents/${content.content_id}/preview`}>
                                <Button size="sm" variant="outline" title="Podgląd">
                            ...
                            <Access allOf={['content.update_any']}>
                              <Link to={`/contents/${content.content_id}/edit`}>
                                <Button size="sm" variant="outline" title="Edytuj">
                ...
                <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex items-center gap-4">
                    <div className="text-sm text-gray-700 dark:text-gray-300">
                      ...
                      {Math.min(currentPage * itemsPerPage, totalItems)} z {totalItems} treści
                    </div>
