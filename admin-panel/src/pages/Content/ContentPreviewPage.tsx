...
export default function ContentPreviewPage() {
  ...
  const fetchAllContent = async () => {
    ...
    try {
      ...
      const allMediaIds = sections.flatMap((s: ContentSection) => s.media_ids || []);
      const uniqueMediaIds = [...new Set(allMediaIds.filter((id: string) => !!id))];
  ...
  const visibleItems = showHidden ? displayItems : displayItems.filter((item) => item.status);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {visibleItems.length} / {displayItems.length} elementów widocznych
              </span>
              <Button
                size="sm"
...
function ComponentRenderer({ component, showHidden, isHidden }: ComponentRendererProps) {
  return (
    <section
      className={`relative ${
        isHidden && showHidden
    ...
    >
      ...
      {component.component_type === 'hero' && <HeroComponent data={component.data} />}
      {component.component_type === 'services' && <ServicesComponent data={component.data} />}
      {component.component_type === 'pricing' && <PricingComponent data={component.data} />}
      {component.component_type === 'team' && <TeamComponent data={component.data} />}
      {component.component_type === 'testimonial' && <TestimonialComponent data={component.data} />}
      {component.component_type === 'hours' && <HoursComponent data={component.data} />}
      {component.component_type === 'contact_form' && (
        <ContactFormComponent data={component.data} />
      )}
      {component.component_type === 'map' && <MapComponent data={component.data} />}
    </section>
  );
}
...
function HeroComponent({ data }: { data: any }) {
  return (
    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-12 rounded-lg">
      <h2 className="text-4xl font-bold mb-4">{data.slides?.[0]?.title || 'Hero'}</h2>
...
function ServicesComponent({ data }: { data: any }) {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Usługi'}</h2>
...
function PricingComponent({ data }: { data: any }) {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Cennik'}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {data.services?.map((service: any, index: number) => (
          <div key={index} className="p-6 border rounded-lg">
            <h3 className="text-xl font-semibold mb-2">{service.name}</h3>
            <p className="text-3xl font-bold text-blue-600 mb-4">{service.price} zł</p>
...
function TeamComponent({ data }: { data: any }) {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Zespół'}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
...
function TestimonialComponent({ data }: { data: any }) {
  return (
    <div className="py-12 bg-gray-50">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Opinie'}</h2>
...
function HoursComponent({ data }: { data: any }) {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Godziny otwarcia'}</h2>
...
function ContactFormComponent({ data }: { data: any }) {
  return (
    <div className="py-12">
      <h2 className="text-3xl font-bold text-center mb-8">{data.title || 'Kontakt'}</h2>
