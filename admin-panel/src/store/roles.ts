...
export const useRoles = create<RolesState>((set, get) => ({
    ...
    updateRole: async (id: string, data: UpdateRolePayload) => {
        ...
        try {
            ...
            const msg =
                e?.response?.data?.error ?? 'Nie udało się zaktualizować roli'
            set({ error: msg, loading: false })
            showAlert({
                variant: 'error',
                title: 'Błąd',
    ...
    fetchAllPermissions: async () => {
        try {
            ...
            const msg =
                e?.response?.data?.error ?? 'Nie udało się pobrać uprawnień'
            set({ error: msg })
            showAlert({
                variant: 'error',
                title: 'Błąd',
    ...
    fetchRolePermissions: async (roleId: string) => {
        try {
            ...
            const msg =
                ...
                'Nie udało się pobrać uprawnień roli'
            set({ error: msg })
            showAlert({
                variant: 'error',
                title: 'Błąd',
    ...
    updateRolePermissions: async (roleId: string, permissionIds: string[]) => {
        try {
            ...
            const msg =
                e?.response?.data?.error ??
                'Nie udało się zaktualizować uprawnień roli'
            set({ error: msg })
            showAlert({
                variant: 'error',
                title: 'Błąd',
