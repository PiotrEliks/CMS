...
interface PageComponentsState {
  ...
  fetchComponents: (contentId: string, includeInactive?: boolean) => Promise<void>;
  createComponent: (
    contentId: string,
    data: {
      component_type: ComponentType;
  ...
  reorderComponents: (contentId: string, componentIds: string[]) => Promise<void>;
...
export const usePageComponents = create<PageComponentsState>((set, get) => ({
  ...
  fetchComponents: async (contentId: string, includeInactive = false) => {
    ...
    try {
      ...
      const res = await api.get(`/components/contents/${contentId}/components`, { params });
  ...
  createComponent: async (contentId, data) => {
    set({ loading: true, error: null });
    try {
      const res = await api.post(`/components/contents/${contentId}/components`, data);
  ...
  deleteComponent: async (componentId) => {
    ...
    try {
      ...
      set((state) => ({
        components: state.components.filter((c) => c.component_id !== componentId),
        loading: false,
      }));
    } catch (error: any) {
      set({ error: error.message, loading: false });
  ...
  reorderComponents: async (contentId, componentIds) => {
    set({ loading: true, error: null });
    try {
      const res = await api.post(`/components/${contentId}/components/reorder`, {
  ...
  duplicateComponent: async (componentId) => {
    ...
    try {
      ...
      set((state) => ({
        components: [...state.components, duplicated].sort((a, b) => a.order_index - b.order_index),
        loading: false,
      }));
  ...
  toggleComponentStatus: async (componentId) => {
    ...
    try {
      ...
      set((state) => ({
        components: state.components.map((c) => (c.component_id === componentId ? toggled : c)),
        loading: false,
      }));
    } catch (error: any) {
      set({ error: error.message, loading: false });
