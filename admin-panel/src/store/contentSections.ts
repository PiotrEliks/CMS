...
export const useContentSections = create<ContentSectionsState>((set, get) => ({
  ...
  createSection: async (contentId, data) => {
    ...
    try {
      ...
      set((state) => ({
        sections: [...state.sections, newSection].sort((a, b) => a.order_index - b.order_index),
        loading: false,
      }));
  ...
  updateSection: async (contentId, sectionId, updates) => {
    set({ loading: true, error: null });
    try {
      const res = await api.put(`/contents/${contentId}/sections/${sectionId}`, updates);
  ...
  deleteSection: async (contentId, sectionId) => {
    ...
    try {
      ...
      set((state) => ({
        sections: state.sections.filter((s) => s.section_id !== sectionId),
        loading: false,
      }));
    } catch (error: any) {
      set({ error: error.message, loading: false });
  ...
  duplicateSection: async (sectionId) => {
    ...
    try {
      ...
      set((state) => ({
        sections: [...state.sections, duplicated].sort((a, b) => a.order_index - b.order_index),
        loading: false,
      }));
  ...
  toggleSectionStatus: async (sectionId) => {
    ...
    try {
      ...
      set((state) => ({
        sections: state.sections.map((s) => (s.section_id === sectionId ? toggled : s)),
        loading: false,
      }));
    } catch (error: any) {
      set({ error: error.message, loading: false });
