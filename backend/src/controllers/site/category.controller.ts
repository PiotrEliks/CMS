...
export class CategoryController {
  ...
  async getContentsByCategory(req: Request, res: Response) {
    try {
      ...
      const category = await categoryService.getContentsByCategory(id, includeInactive);
      res.json(category);
    } catch (error: any) {
      res.status(404).json({ error: error.message });
    }
  ...
  async assignCategoriesToContent(req: Request, res: Response) {
    try {
      ...
      const content = await categoryService.assignCategoriesToContent(contentId, category_ids);
      res.json(content.categories || []);
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  async removeCategoryFromContent(req: Request, res: Response) {
    try {
      ...
      const result = await categoryService.removeCategoryFromContent(contentId, categoryId);
      res.json(result);
    } catch (error: any) {
      res.status(404).json({ error: error.message });
    }
