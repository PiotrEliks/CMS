    ...
    const images: SeedMedia[] = [
        {
            ...
            alt_text: 'Hero background 1',
            title: 'Hero Background',
        },
        {
            filename: 'hero_bg_2.jpg',
            alt_text: 'Hero background 2',
...
export async function ensureHomepageSeed() {
    ...
    const getMediaUrl = (filename: string) =>
        mediaUrls.get(filename) || `/images/${filename}`
...
export async function ensureAdminSeed() {
    ...
    await sequelize.transaction(async (t) => {
        ...
        if (!existingModerator) {
            ...
            console.log(
                `✅ Seeded moderator: ${moderatorEmail} / ${moderatorPlain}`
            )
        } else {
