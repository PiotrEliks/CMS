...
export class PageComponentService {
    ...
    async createComponent(data: {
        ...
        if (data.display_order === undefined) {
            ...
            data.display_order =
                lastDisplay && lastDisplay.display_order !== null
                    ? lastDisplay.display_order + 1
                    : 0
        }
    ...
    async deleteComponent(componentId: string) {
        ...
        if (deletedDisplay !== null && deletedDisplay !== undefined) {
            await PageComponent.update(
                {
                    display_order:
                        PageComponent.sequelize!.literal('display_order - 1'),
                },
                {
                    where: {
    ...
    async duplicateComponent(componentId: string) {
        ...
        await PageComponent.update(
            {
                order_index:
                    PageComponent.sequelize!.literal('order_index + 1'),
            },
            {
                where: {
                    content_id: original.content_id,
