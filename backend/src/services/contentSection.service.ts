...
import { ContentSection, SectionType, SectionSettings } from '../models/contentSection.model.js';
...
export class ContentSectionService {
  ...
  async deleteSection(sectionId: string) {
    ...
    await ContentSection.update(
      { order_index: (ContentSection as any).sequelize.literal('order_index - 1') },
      {
        where: {
